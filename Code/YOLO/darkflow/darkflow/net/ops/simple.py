...
class route(BaseOp):
    def forward(self):
        ...
        for r in routes:
            ...
            routes_out.append(this.out)
        self.out = tf.concat(routes_out, axis=3)
...
class connected(BaseOp):
    def forward(self):
        self.out = tf.nn.bias_add(tf.matmul(self.inp.out, self.lay.w['weights']), self.lay.w['biases'])
...
class flatten(BaseOp):
    def forward(self):
        ...
        self.out = tf.reshape(temp, [tf.shape(temp)[0], -1])
...
class avgpool(BaseOp):
    def forward(self):
        self.out = tf.reduce_mean(self.inp.out, axis=[1, 2])
...
class dropout(BaseOp):
    def forward(self):
        ...
        self.out = tf.nn.dropout(self.inp.out, rate=1-self.lay.h['pdrop'])
...
class maxpool(BaseOp):
    def forward(self):
        self.out = tf.nn.max_pool2d(self.inp.out, ksize=[self.lay.ksize, self.lay.ksize], strides=[self.lay.stride, self.lay.stride], padding='SAME')
...
class leaky(BaseOp):
    def forward(self):
        self.out = tf.nn.leaky_relu(self.inp.out, alpha=0.1)
